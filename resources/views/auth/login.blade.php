<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="mt-4" x-data="{ showPassword: false }">
            ...
            <div class="relative w-full mt-1" style="position: relative;">
                <x-text-input id="password" class="block w-full pr-10" x-bind:type="showPassword ? 'text' : 'password'"
                    name="password" required autocomplete="current-password" />

                <button type="button" @click="showPassword = !showPassword"
                    style="position: absolute; right: 12px; top: 0; bottom: 0; display: flex; align-items: center; z-index: 10;"
                    class="text-muted-foreground hover:text-foreground focus:outline-none transition-colors">
                    <svg x-show="!showPassword" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5"
