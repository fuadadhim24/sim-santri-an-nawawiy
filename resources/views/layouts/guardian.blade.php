...
<body class="font-sans antialiased bg-background text-foreground" x-data="{ openHelp: false }">
        ...
        <aside id="sidebar"
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                ...
                <a href="{{ route('wali.payment-history') }}"
                    class="flex items-center px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('wali.payment-history') ? 'bg-sidebar-primary text-sidebar-primary-foreground' : 'text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground' }} group transition-colors">
                    <svg class="mr-3 h-5 w-5 {{ request()->routeIs('wali.payment-history') ? 'text-white' : 'text-sidebar-foreground group-hover:text-sidebar-accent-foreground' }}"
                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div class="border-t border-sidebar-border p-4 space-y-1">
                <a href="{{ route('wali.profile.edit') }}"
                    class="flex w-full items-center px-2 py-2 text-sm font-medium text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground rounded-md transition-colors {{ request()->routeIs('wali.profile.edit') ? 'bg-sidebar-accent text-sidebar-accent-foreground' : '' }}">
    ...
    <div class="relative">
        ...
        <!-- Drawer Panel -->
        <div x-show="openHelp" style="display: none;"
