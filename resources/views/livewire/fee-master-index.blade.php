<div>
            ...
            <div class="flex items-center space-x-2 flex-nowrap">
                <select wire:model.live="categoryFilter"
                    ...
                    @foreach ($this->feeCategories as $category)
                        <option value="{{ $category->id }}">{{ $category->name }}{{ !$category->is_active ? ' (Non-aktif)' : '' }}</option>
                    @endforeach
                </select>
                    ...
                    @forelse ($fees as $fee)
                        <tr class="hover:bg-muted/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap w-px space-x-2">
                                ...
                                <button type="button"
                                    wire:click="confirmDelete({{ $fee->id }})"
    ...
    <script>
        ...
        $wire.on('show-delete-confirm-modal', (event) => {
            const data = event[0] || event;
            window.Swal.fire({
        ...
        $wire.on('show-simple-delete-confirm-modal', (event) => {
            const data = event[0] || event;
            window.Swal.fire({
                title: 'Arsipkan Master Biaya?',
                text: `Apakah Anda yakin ingin mengarsipkan Master Biaya "${data.itemName}"?`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
